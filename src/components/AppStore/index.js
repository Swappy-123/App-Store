...
class AppStore extends Component {
  ...
  clickChange = value => {
    this.setState({activeAppId: value})
  }

  getFilter = () => {
    const {activeAppId} = this.state

    const filteredList = appsList.filter(each => each.category === activeAppId)
    return filteredList
  }

  getSearch = () => {
    const {searchInput, activeAppId} = this.state
    const searchFilter = appsList.filter(
      eachItem =>
        eachItem.appName.toLowerCase().includes(searchInput.toLowerCase()) &&
        eachItem.category === activeAppId,
    )
    return searchFilter
  }
  ...
  render() {
    const {activeAppId} = this.state
    const filteredList = this.getFilter()
    const searchFilter = this.getSearch()
    return (
      <div className="bg-container">
        <h1 className="head">App Store</h1>
        <div className="inputCon">
          <input className="inp" onChange={this.changeValue} type="search" />
          <img
        ...
        <ul className="tabs">
          {tabsList.map(tabDetails => (
            <TabItem
              ...
              isActive={activeAppId === tabDetails.tabId}
            />
          ))}
        </ul>
        <ul className="apps">
          {searchFilter.map(appDetails => (
